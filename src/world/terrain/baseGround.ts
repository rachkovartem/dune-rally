...
// pan, the border ranges and the NW gorge. Roads and pads are graded from this ground, and the
// river's bed is measured from it.
import { lerp, smoothstep } from '../blend';
import { microRelief, plainHeight } from './basePlain';
import { applyLandforms } from './landforms';
...
export function baseGroundHeight(x: number, z: number): number {
  ...
  const raised = applyLandforms(natural, x, z).height + duneRise(x, z);
