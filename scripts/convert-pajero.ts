// scripts/convert-pajero.ts
// Converts the owner's Pajero Sport gen 3 facelift GLB (3D Warehouse) into
// public/models/pajero-sport-2020.glb, which is gitignored (see public/models/SOURCE.md).
// Run: npx tsx scripts/convert-pajero.ts "<dir>/model.glb"
...
import {
  ...
  pajeroPlateQuad,
  pajeroRawKeyOf,
  type PajeroDeletionReason,
  type PajeroTargetKey,
  type PajeroVector,
} from '../src/assets/pajeroGen3PartRules';
...
const OUTPUT_PATH = path.join(repoRoot, 'public', 'models', 'pajero-sport-2020.glb');
...
/** Real Pajero Sport gen 3 facelift (approximate), for the scale check only. */
const REAL_WHEELBASE = 2.8;
const REAL_LENGTH = 4.825;
...
function carSpaceSoupOf(node: GltfNode, primitive: Primitive): TriangleSoup {
  ...
  if (!position) throw new Error(`convert-pajero: a primitive of "${node.getName()}" has no POSITION`);
  ...
  for (let vertex = 0; vertex < position.getCount(); vertex++) {
    ...
    const car = pajeroCarSpacePoint(world);
...
function boundsOf(positions: Float32Array): { centre: PajeroVector; size: PajeroVector } {
...
function withoutBadges(rawKey: string, soup: TriangleSoup, deletedTriangles: Map<PajeroDeletionReason, number>): TriangleSoup {
  if (!PAJERO_BADGE_COMPONENTS[rawKey]) return soup;
  ...
  for (const component of components) {
    ...
    const reason = pajeroBadgeReasonOf(rawKey, box);
    if (!reason) continue;
    removed.add(component.id);
    deletedTriangles.set(reason, (deletedTriangles.get(reason) ?? 0) + component.triangles);
  }
  if (removed.size === 0) throw new Error(`convert-pajero: the badge rule for "${rawKey}" matched nothing`);
  const kept: number[] = [];
...
function primitiveFromSoup(doc: Document, soup: TriangleSoup, hub: PajeroVector | null): Primitive {
...
function soupOfPrimitive(primitive: Primitive): TriangleSoup {
  ...
  if (!position || !indices) throw new Error('convert-pajero: a joined primitive is not indexed');
  const positions = new Float32Array(position.getCount() * 3);
  const element = [0, 0, 0];
...
/** Appends a reversed copy of every triangle the cabin-shell rule names; returns how many. */
function addInsideFaces(doc: Document, primitive: Primitive, slot: CarMaterialSlot): number {
  const soup = soupOfPrimitive(primitive);
  const added: number[] = [];
  for (let triangle = 0; triangle < soup.indices.length / 3; triangle++) {
    const corners = [soup.indices[triangle * 3], soup.indices[triangle * 3 + 1], soup.indices[triangle * 3 + 2]];
    const centre = { x: 0, y: 0, z: 0 };
    for (const vertex of corners) {
      centre.x += soup.positions[vertex * 3] / 3;
      centre.y += soup.positions[vertex * 3 + 1] / 3;
      centre.z += soup.positions[vertex * 3 + 2] / 3;
    }
    if (pajeroNeedsInsideFace(slot, centre)) added.push(corners[0], corners[2], corners[1]);
  }
  if (added.length === 0) return 0;
  const indices = new Uint32Array(soup.indices.length + added.length);
  indices.set(soup.indices);
  indices.set(added, soup.indices.length);
  primitive.setIndices(doc.createAccessor().setType('SCALAR').setArray(indices).setBuffer(doc.getRoot().listBuffers()[0]));
...
function requireNode(nodes: ReadonlyMap<string, GltfNode>, cleanId: string): GltfNode {
  ...
  if (!node) throw new Error(`convert-pajero: the converted model has no "${cleanId}" node`);
  return node;
}

function localPositionsOf(node: GltfNode): Float32Array {
  const primitive = node.getMesh()?.listPrimitives()[0];
  if (!primitive) throw new Error(`convert-pajero: node "${node.getName()}" has no primitive`);
  return soupOfPrimitive(primitive).positions;
}

/** Reads `measuredCarPajeroGen3` off the converted document. Run it before meshopt, which quantises
...
function printMeasuredCar(doc: Document, nodes: ReadonlyMap<string, GltfNode>): void {
  ...
  // Arch top: the lowest paint point straight above the front hub. The arch lip sits outboard of the
  // hub, so the window is wider across than along; a longer one would catch the lower arch flank.
  const paint = localPositionsOf(requireNode(nodes, 'paint'));
  let archTopY = Infinity;
  for (let offset = 0; offset < paint.length; offset += 3) {
    const isAboveHub = Math.abs(paint[offset] - hub[0]) < 0.2 && Math.abs(paint[offset + 2] - hub[2]) < 0.05;
  ...
  for (const cleanId of ['paint', 'blackTrim', 'silver']) {
  ...
  console.log('Measured (from the converted GLB, metres) — copy into measuredCarPajeroGen3:');
  for (const [name, value] of rows) {
    console.log(`  ${name.padEnd(13)}${Number.isFinite(value) ? value.toFixed(4) : 'not found'}`);
  }
  console.log('Other sizes (metres, not in measuredCarPajeroGen3):');
  ...
  console.log(
    `Scale check: wheelbase ${((wheelbase / REAL_WHEELBASE - 1) * 100).toFixed(2)} % and length ${((length / REAL_LENGTH - 1) * 100).toFixed(2)} % against the real car (${REAL_WHEELBASE} / ${REAL_LENGTH} m)`,
  );
}
...
async function main(): Promise<void> {
  ...
  if (!sourcePath) {
    console.error('Usage: npx tsx scripts/convert-pajero.ts "<dir>/model.glb"');
    process.exit(1);
  }
  ...
  const deletedTriangles = new Map<PajeroDeletionReason, number>();
  ...
  for (const node of source.getRoot().listNodes()) {
    ...
    for (const primitive of mesh.listPrimitives()) {
      const rawKey = pajeroRawKeyOf(pathFromSceneOf(node), primitive.getMaterial()?.getName() ?? null);
      seenKeys.add(rawKey);
      const soup = carSpaceSoupOf(node, primitive);
      rawTriangles += soup.indices.length / 3;
      const decision = classifyPajeroPart({ rawKey, ...boundsOf(soup.positions) });
      ...
      const kept = withoutBadges(rawKey, soup, deletedTriangles);
      const group = groups.get(decision.cleanId) ?? { slot: decision.slot, targetKey: decision.targetKey, hub: decision.hub, primitives: [], rawParts: 0 };
      group.primitives.push(primitiveFromSoup(doc, kept, decision.hub));
      group.rawParts++;
      groups.set(decision.cleanId, group);
    }
  }
  const unusedKeys = Object.keys(PAJERO_RAW_ROLES).filter((rawKey) => !seenKeys.has(rawKey));
  if (unusedKeys.length > 0) {
    throw new Error(`convert-pajero: rules for raw keys the source does not have: ${unusedKeys.join(', ')}`);
  }
  ...
  const insideFaceLines: string[] = [];
  const cleanNodes = new Map<string, GltfNode>();
  const addCleanNode = (cleanId: string, primitive: Primitive, hub: PajeroVector | null): void => {
    const cleanNode = doc
      .createNode(cleanId)
  ...
  for (const [cleanId, group] of groups) {
    if (pajeroMaterialSlotFor(cleanId) !== group.slot) {
      throw new Error(`convert-pajero: "${cleanId}" was classified as ${group.slot}, the rules say ${pajeroMaterialSlotFor(cleanId)}`);
    }
    ...
    const target = PAJERO_TRIANGLE_TARGETS[group.targetKey];
    ...
    // Body parts only: they keep car space, which the cabin box is written in.
    const insideFaces = group.hub ? 0 : addInsideFaces(doc, joined, group.slot);
    if (insideFaces > 0) insideFaceLines.push(`${cleanId} +${insideFaces}`);
  ...
  const plateQuad = pajeroPlateQuad();
  const plate = primitiveFromSoup(doc, plateQuad, null).setMaterial(materialFor('plate'));
  addCleanNode('plate', plate, null);
  report.push({ cleanId: 'plate', slot: 'plate', rawParts: 0, before: 0, target: PAJERO_TRIANGLE_TARGETS.plate, after: 2 });
  ...
  if (!paint) throw new Error('convert-pajero: the paint node has no primitive');
  ...
  console.log(`Deleted triangles (${[...deletedTriangles.values()].reduce((sum, count) => sum + count, 0)}):`);
  ...
  console.log(`Inside faces added (counted in "after"): ${insideFaceLines.join(', ')}`);
