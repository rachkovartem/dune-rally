...
// Prints the car comparison numbers (spec C2.6–C2.12) for both cars on road and sand grip, the
// real-gravity checks of plan v3 S0-4, and a PASS/FAIL line per target. Exit code 1 on any FAIL.
...
import {
  createBenchCar, runBraking, runCrest, runDropSettle, runHillClimb, runRollover, runStraightLine, runTurn,
  speedAt, stepBenchCar,
...
const results: Record<string, { timeTo100: number | null; topSpeed: number; rollAt60: number; flipped: boolean }> = {};
...
const DEGREES_LIMIT: Record<CarId, number> = { forester: 4, pajero: 6 };
const ZERO_TO_100: Record<CarId, [number, number]> = { forester: [8.5, 9.5], pajero: [11.0, 12.5] };
const TOP_SPEED: Record<CarId, [number, number]> = { forester: [185, 200], pajero: [170, 180] };
const RESTING_TARGET: Record<CarId, number> = { forester: 0.481, pajero: 0.353 };
for (const carId of CAR_IDS) {
